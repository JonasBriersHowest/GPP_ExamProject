...
## Original idea
The original idea of the project was to implement Goal Oriented Action Planning (GOAP).
...
This made me think if I could adapt GOAP to meet the needs of this project better.
## The solution
...
I made some important changes to GOAP:
 - A worldState could contain anything (bools, ints, floats or even a Vector2 or string).
 - Instead of a clear goal I will define a function that evaluates a worldState and gives it a score.
 - It will use a depth first algorithm to determine the path with the highest possible score.
Because these changes seemed to different from GOAP I decided to rename the algorithm to "Abstract Goal Action Planning (AGAP)".
I have tried to find if a similar algorithm already exists, but I couldn't find it (it's difficult to look for something if you don't know the name).
## Implementation
### World State
I have a very simple implementation of a worldState: it's just a struct containing all the variables that I need. The reason I do this is because I want to easily ba able to copy and compare my worldState.
```cpp
...
```
### Planning
For my planning I use a recursive depth first search algorithm that finds the best possible worlState (pseudocode because actual code is very long and contains many checks):
```
...
While it was very fun and inte# Abstract Goal Action Planning - Jonas Briers
## Original idea
The original idea of the project was to implement Goal Oriented Action Planning (GOAP).
GOAP is able to determine the most efficient way to achieve a certain goal.
It does this by creating a navmesh where all nodes are the different actions the agent can do. 
...
## Implementation
### World State
I have a very simple implementation of a worldState: it's just a struct containing all the variables that I need. The reason I do this is because I want to easily be able to copy and compare my worldState.
```cpp
// Just an example, my actual worldState contains more variables
struct WorldState
{
...
While it was very fun and interesting to try to come up with my own AI system based on AGAP, I don't think it's gonna be the next big thing in the industry.
